...
export class SupplierInfo extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public LoadUserDetails() {
    this.IsLoading = true;
    this.userSVC.GetUserDetails(this.UserIncludes.join(','), false).subscribe((res: any) => {
  ...
  public LoadUserDocuments() {
    ...
    this.userSVC.FindAllMyDocuments(false).subscribe((res: any) => {
      this.Documents = res;
      ...
      this.Certificates = this.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.Certificate);
  ...
  public OnSaveDetails() {
    ...
    this.IsLoading = true;
    this.userSVC.UpdateSupplierInfo(this.User, false).subscribe((res: any) => {
    ...
    this.sharedSVC.UploadFile({
      ...
      switch (type) {
        ...
        case AppEnums.DocumentType.NOL:
          ...
          break;
        case AppEnums.DocumentType.Certificate:
          let Cer: any = {
            DocumentType: AppEnums.DocumentType.Certificate,
            UserId: this.ActiveUser.Id
          }

          Cer.File = environment.MainEndPoint + res;
          Cer.FileName = event.target.files[0].name;
          Cer.FileSize = event.target.files[0].size;

          this.userSVC.AddUserDocument(Cer).subscribe((res: any) => {
  ...
  public async OnUploadImage(event: any) {
    ...
    this.sharedSVC.UploadFile({
      ...
      this.User.Image = environment.MainEndPoint + res;
      this.User.State = AppEnums.BaseState.Modified;
  ...
  public ValidateWorkingDays() {
    ...
    this.DaysSelections.forEach((x: any) => {
      ...
      if (x.WorkingDay) {
        if (x.WorkingDay.State == AppEnums.BaseState.Unchanged)
          x.WorkingDay.State = AppEnums.BaseState.Modified;
    ...
    if (!found)
      this.UserValidation.NoDaySelected = true;
  ...
  public OnDeleteCertificateConfirmation(cer: any) {
    ...
    Swal.fire({
      ...
      if (res.value) {
        this.OnDeleteCertificate(cer);
      }
    })

  }

  public OnDeleteCertificate(cer: any) {
    this.userSVC.DeleteUserDocument(cer.Id, false).subscribe((res: any) => {
      this.LoadUserDocuments();
    });
  }
}
