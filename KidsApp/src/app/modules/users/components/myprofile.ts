...
export class MyProfile extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public CheckUser() {
    ...
    if (this.ActiveUser.UserType == AppEnums.UserTypes.CompanySupplier || this.ActiveUser.UserType == AppEnums.UserTypes.IndividualSupplier)
      this.navigate('');
  }
  ...
  public LoadKids() {
    ...
    this.userSVC.FindAllKids(Cri, -1, -1, null, false).subscribe((res: any) => {
      ...
      if (this.Kids) {
        this.Kids.forEach((x: any) => {
          if (x.DOB)
            x.DOB = SharedService.ResponeToDate(x.DOB);
        })
      }
