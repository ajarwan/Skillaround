...
export class AppHeader extends BaseComponent implements OnInit, AfterViewInit {
  ...
  get PersonalPhotoName() {
    if (!this.User || !this.User.Documents)
      return null;
  ...
  get GovermentIdName() {
    ...
    let photo = this.User.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.GovermentId)[0];
  ...
  get CertificateName() {
    ...
    let photo = this.User.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.Certificate)[0];
  ...
  get NOLName() {
    ...
    let photo = this.User.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.NOL)[0];
  ...
  get SignedDocumentName() {
    if (!this.User || !this.User.Documents)
      return null;

    let photo = this.User.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.SignedDocument)[0];
  ...
  get TradeLicenceName() {
    if (!this.User || !this.User.Documents)
      return null;

    let photo = this.User.Documents.filter((x: any) => x.DocumentType == AppEnums.DocumentType.TradeLicence)[0];
    ...
    switch (tab) {
      ...
      case BecomeSupplierTabs.Details:
        ...
        if (this.User.UserType == AppEnums.UserTypes.IndividualSupplier) {
          ...
          if (Object.getOwnPropertyNames(this.UserValidation).length > 0) {
            navigate = false;
            if (!this.UserValidation.PasswordLength) {
              this.UserValidation.PleaseFillInRequiredFields = true;
            }
          }
