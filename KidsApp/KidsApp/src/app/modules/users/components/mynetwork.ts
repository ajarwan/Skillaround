...
export class MyNetwork extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public IsLoadingMoreSuggestedUsers = false;
  ...
  public OnSearchSuggestedUsers() {
    ...
    this.SuggestedUsers = []
    this.LoadSuggestedUsers(this.IsLoadingSuggestedUsers);
  }

  public LoadSuggestedUsers(flag: any) {
    flag = true;
  ...
  public LoadMoreSuggestedUsers() {
    this.SuggestedUsersPager.PageIndex += 1;
    this.LoadSuggestedUsers(this.IsLoadingMoreSuggestedUsers);
  }

  public OnConnect(user) {
    if (user.ConnectDisabled)
      return;

    user.ConnectDisabled = true;
    let UserFrind: any = {
      UserId: this.ActiveUser.Id,
      FriendId: user.Id
    };

    this.userSVC.ConnectUsers(UserFrind).subscribe((res: any) => {
