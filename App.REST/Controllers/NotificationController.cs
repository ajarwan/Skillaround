...
namespace App.REST.Controllers
{
    ...
    public class NotificationController : BaseController
    {
        ...
        [Route("SetSeenNotification")]
        [HttpGet]
        [KidsAppAuthorization]
        public IHttpActionResult SetNotificationsSeen()
        {
            try
            {
                Expression<Func<Notification, bool>> Filter = x => !x.IsDeleted && x.ToUserId == this.ActiveUser.Id;
                Mgr.UpdateBatch(Filter, new Dictionary<string, string> { { EntityDescriptor<Notification>.Property(y => y.IsSeen), "1" } });
