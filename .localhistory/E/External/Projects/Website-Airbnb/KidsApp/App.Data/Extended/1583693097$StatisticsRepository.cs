...
namespace App.Data.Extended
{
    public class StatisticsRepository : RepositoryBase<Statistic>
    {
        ...
        public async Task<List<UserDTO>> FindMostActiveUsers()
        {
            ...
            var res = await Query.OrderByDescending(x => x.CreateDate).GroupBy(x => x.UserId).OrderBy(x => x.Count()).ThenByDescending(x => x.FirstOrDefault().CreateDate).Select(x => x.FirstOrDefault()).Select(x => new UserDTO()
            {
