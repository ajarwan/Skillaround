...
namespace App.Data.Extended
{
    public class StatisticsRepository : RepositoryBase<Statistic>
    {
        ...
        public async Task<List<UserDTO>> FindMostActiveUsers()
        {
            ...
            var res = await Query.OrderByDescending(x => x.CreateDate).GroupBy(x => x.UserId).OrderBy(x => x.Count()).Select(x => x.FirstOrDefault()).Select(x => new UserDTO()
            {
        ...
        public async Task<List<Activity>> FindMostViewedActivities()
        {
            ...
            var res = await Query.OrderByDescending(x => x.Type).GroupBy(x => x.ActivityId).OrderBy(x => x.Count()).Select(x => x.FirstOrDefault()).Select(x => new Activity()
            {
                Id = x.ActivityId.Value,
                TitleAr = x.Activity.TitleAr,
                TitleEn = x.Activity.TitleEn,
                Category = x.Activity.Category,
                From = x.Activity.From,
                To = x.Activity.To,
                LocationName = x.Activity.LocationName,
                SupplierId = x.Activity.SupplierId,
            }).Take(6).ToListAsync();
