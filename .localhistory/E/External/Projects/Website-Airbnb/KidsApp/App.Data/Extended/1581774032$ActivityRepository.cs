...
namespace App.Data.Extended
{
    public class ActivityRepository : RepositoryBase<Activity>
    {
        ...
        public bool CheckAvailability(int ActivityId, int count)
        {
            ...
            {
                var available = dbSet.Where(x => !x.IsDeleted && x.Id == ActivityId).Select(x => x.Capacity - x.Bookings.Where(y => !y.IsDeleted && y.Status != Entity.SharedEnums.BookingConfirmationStatus.Cancelled).Select(y => y.Count).Sum()).FirstOrDefault()
