...
namespace App.Data.Extended
{
    public class StatisticsRepository : RepositoryBase<Statistic>
    {
        ...
        public async Task<List<Activity>> FindMostViewedActivities()
        {
            ...
            var res = await Query.OrderByDescending(x => x.Type).GroupBy(x => x.ActivityId).OrderByDescending(x => x.Count())
                .Select(x => x.FirstOrDefault().Activity).Include(y=>y.Category).Take(6).ToListAsync();
