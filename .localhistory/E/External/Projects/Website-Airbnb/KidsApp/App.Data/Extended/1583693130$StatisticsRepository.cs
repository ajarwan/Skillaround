...
namespace App.Data.Extended
{
    public class StatisticsRepository : RepositoryBase<Statistic>
    {
        ...
        public async Task<List<UserDTO>> FindMostActiveUsers()
        {
            ...
            var res = await Query.OrderByDescending(x => x.CreateDate).GroupBy(x => x.UserId).OrderBy(x => x.Count()).ThenByDescending(x => x.LastOrDefault().CreateDate).Select(x => x.LastOrDefault()).Select(x => new UserDTO()
            {
