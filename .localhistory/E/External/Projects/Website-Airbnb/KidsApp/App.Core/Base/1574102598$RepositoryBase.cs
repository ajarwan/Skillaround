...
namespace App.Core.Base
{
    ...
    public class RepositoryBase<TEntity> where TEntity : EntityBase
    {
        ...
        )
        {
            ...
            query = GetPagerBy(query, pagerBy, orderBy);
      ...
      (
        ...
        public IQueryable<TEntity> GetPagerBy(IQueryable<TEntity> query, Pager pager, List<OrderBy<TEntity>> orderBy)
        {
        ...
        public IQueryable<TEntity> GetPagerBy(IQueryable<TEntity> query, Pager pager, string orderBy)
        {
            if (pager != null && pager.PageIndex > 0 && pager.PageSize > 0)
            {
                pager.TotalRecords = query.Select(x => x.Id).Count();
