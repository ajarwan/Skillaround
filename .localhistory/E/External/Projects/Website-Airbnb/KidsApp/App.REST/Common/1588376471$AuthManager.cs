...
namespace App.REST.Common
{
    public class AuthManager
    {
        ...
        public static string GenerateToken(User user, int expireMinutes = 2)
        {
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new[]
                {
                  ...
                  new Claim("UserType",Convert.ToInt32(user.UserType).ToString()),
        ...
        public static ClaimsPrincipal GetPrincipal(string token)
        {
            ...
            try
            {
                ...
                userId = int.Parse(jwtToken.Claims.Where(x => x.Type == "Id").FirstOrDefault().Value);
                useremail = jwtToken.Claims.Where(x => x.Type == "Email").FirstOrDefault().Value;
                isSupplier = jwtToken.Claims.Where(x => x.Type == "IsSupplier").FirstOrDefault().Value;
                userType = int.Parse(jwtToken.Claims.Where(x => x.Type == "UserType").FirstOrDefault().Value);
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("The token is expired"))
                {
                    User u = new User()
                    {
                        Id = userId,
                        Email = useremail,
                        IsSupplier = Convert.ToBoolean(isSupplier),
                        UserType = (Entity.SharedEnums.UserTypes)(userType)

                    };

                    var newToken =  AuthManager.GenerateToken(u);
                    HttpContext.Current.Response.Headers.Add("X-Token", newToken);
                    newToken = "Bearer " + newToken;
                    return AuthManager.GetPrincipal(newToken);
                }
                return null;
            }
        }
