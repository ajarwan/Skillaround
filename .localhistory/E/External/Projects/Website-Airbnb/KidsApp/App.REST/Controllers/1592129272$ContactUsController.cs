...
namespace App.REST.Controllers
{
    ...
    public class ContactUsController : BaseController
    {
            ...
            int pageIndex = 1, int pageSize = 10, string include = null, string orderBy = "IsSeen ASC, CreateDate DESC",string KeyWord= null)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(KeyWord))
                {
                    ...
                    Filter = Filter.And(x => x.Messgae.ToLower().Contains(KeyWord) || x.Email.ToLower().Contains(KeyWord) || x.MobileNumber.ToLower().Contains(KeyWord));
        ...
        [HttpPut]
        [KidsAppAuthorization]
        public IHttpActionResult AddContactUsMessage(ContactUsMessage ContactUsMessage)
        {
            try
            {
                ...
                ContactUsMessage.State = BaseState.Modified;
                Mgr.AddUpdate(ContactUsMessage);
                Unit.SaveChanges();
        ...
        [HttpGet]
        [KidsAppAuthorization]
        public async Task<IHttpActionResult> FindContacUsMessagesCount()
        {
            try
            {
                ...
                Expression<Func<ContactUsMessage, bool>> Filter = x => !x.IsDeleted && !x.IsSeen;


                return Ok(await Mgr.Count_Async(Filter));
            }
            catch (Exception ex)
            {
                Unit.LogError(ex, this.ToString(), this.ActionContext.ActionArguments.ToList());
