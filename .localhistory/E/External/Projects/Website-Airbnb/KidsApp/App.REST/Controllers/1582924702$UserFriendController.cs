...
namespace App.REST.Controllers
{
    ...
    public class UserFriendController : BaseController
    {
        ...
        [HttpPut]
        [KidsAppAuthorization]
        public async Task<IHttpActionResult> OnConnectionApproval(int userFriendId, SharedEnums.AcceptStatus status)
        {
            try
            {
                //What abou double checking the relation if exisit already ? 
                UserFriends userFriend = await this.Mgr.FindById_Async(userFriendId);
                userFriend.State = BaseState.Modified;
                userFriend.AcceptStatus = status;
                Mgr.AddUpdate(userFriend);
