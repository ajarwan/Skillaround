 ...
 ****************************/
  ...
  public OnBecomeSupplierRegister() {
    ...
    if (!this.User.UserDocument) {
      ...
      if (this.User.UserDocument.filter((x: any) => x.DocumentType == AppEnums.DocumentType.PersonalPhoto).length == 0)
        this.UserValidation.PersonalPhoto = true;
      if (this.User.UserDocument.filter((x: any) => x.DocumentType == AppEnums.DocumentType.GovermentId).length == 0)
        this.UserValidation.GovermentId = true;
      if (this.User.UserDocument.filter((x: any) => x.DocumentType == AppEnums.DocumentType.TradeLicence).length == 0)
        this.UserValidation.TradeLicence = true;
    }
