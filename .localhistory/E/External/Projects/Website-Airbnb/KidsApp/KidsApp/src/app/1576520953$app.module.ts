...
@NgModule({
  ...
  providers: [
    ...
    { provide: APP_INITIALIZER, useFactory: LoadUser, deps: [UsersService, HttpClient], multi: true },
...
export function LoadSettings(CoreSVC: CoreService) {
  ...
  return () => CoreSVC.LoadAppSetting(styles, scripts, 'assets/images/logo.png').then(() => {

  });
}

export function LoadUser(CoreSVC: CoreService, http: HttpClient) {
  ...
  if (token) {
    ...
    http.get(window['_Config'].MainEndPoint, options)
  }
  else
    return () => null;
