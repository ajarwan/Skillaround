  ...
  public FindAllBordersMarks() {
    ...
    this.activitySVC.FindAllMapMarks(this.FinalCriteria).subscribe((res: any) => {
      ...
      if (res.d && res.d.length > 0) {
        ...
        MarksData = MarksData.filter((x: any) => this.CurrentMarksData.find((y) => y.Id == x.Id) == null);
        let RemovedIds = this.CurrentMarksData.filter((x: any) => MarksData.find((y) => y.Id == x.Id) == null);
