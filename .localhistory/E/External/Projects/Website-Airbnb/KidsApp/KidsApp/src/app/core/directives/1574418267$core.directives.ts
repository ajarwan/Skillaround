...
@Directive({
  selector: '[cpcClick]',
  host: { '[class.pointer]': '!cpcDisabled' , '[class.disabled]': 'cpcDisabled' }

})
export class CPCClick {

  /*****************************
   *    Properties
   ****************************/
  private elem: HTMLElement;
  @Output() cpcClick: EventEmitter<any> = new EventEmitter<any>();
  @Input() cpcDisabled: boolean = false;
  @Input() cpcResult: boolean = false;
  ...
  clickHandler(e: any): boolean {
    ...
    this.cpcClick.emit(e);

    return this.cpcResult;
  }
...
@Directive({
  selector: '[cpcHidden]',
  host: { '[class.cpc-hidden]': 'cpcHidden' }
})
export class CPCHidden {
  /*****************************
   *          Members
   *****************************/
  @Input() cpcHidden: boolean;
  private elem: HTMLElement;
...
*****************************/
@Directive({
  selector: '[cpcCharLimit]',
  providers: [NgModel],
  host: {
    '(keydown)': 'onValueChange($event)'
  }
})
export class CPCCharLimit {
  /*****************************
   *          Members
   *****************************/
  @Input() cpcCharLimit: CoreEnums.CharLimit;
  private elem: HTMLElement;
...
export class CPCEnter {
  /*****************************
   *          Members
   *****************************/
  private elem: HTMLElement;
  @Output() cpcEnter: EventEmitter<any> = new EventEmitter<any>();
...
*****************************/
@Directive({
  selector: '[cpcImageError]'
})
export class CPCImageError implements OnInit {
  /*****************************
   *          Members
   *****************************/
  private elem: HTMLElement;
  ...
  @Input('cpcImageError') cpcImageError: string;
  ...
  ngOnInit() {
    ...
    if (!(<HTMLImageElement>this.elem).src || (<HTMLImageElement>this.elem).src.indexOf('null') > -1)
      (<HTMLImageElement>this.elem).src = this.cpcImageError;
    ...
    switch (this.elem.tagName.toLowerCase()) {
      case 'img':
        ...
        break;
      default:
        throw new Error("CPCImageError Directive apply only to image tag.");
    }
  }
