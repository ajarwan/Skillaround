...
export class ActivityService {
  ...
  public FindAllActivities(criteria: any, pageIndex: number = -1, pageSize: number = -1, orderBy = null, include: string = null, showSpinner: boolean = true): Observable<Response> {
    ...
    if (environment.IsDesignerVesion) {
      return of(<any>({ d: TestDataService.Activities, pager: new Pager(1, 2, 3, 8) })).pipe(delay(500));
    }
  ...
  public FindAllMapMarks(criteria: any): Observable<Response> {

    if (environment.IsDesignerVesion) {
      return of(<any>{ d: TestDataService.MapMarks, pager: new Pager() }).pipe(delay(500));
    }
    ...
    return this.http.get(new UrlConfig(`${this.ACTIVITY_BASE_URL}FindAllMarks${params.getQuery()}`, this.mainEndPoint), false);
  }

  public FindAvailableCapacityCount(id: number, showSpinner: boolean = true): Observable<Response> {

    if (environment.IsDesignerVesion) {
      return of(<any>(5)).pipe(delay(500));
    }
    var params = new SearchParams();


    params.set('activityId', id.toString())
    return this.http.get(new UrlConfig(`${this.ACTIVITY_BASE_URL}AvailableCapacity${params.getQuery()}`, this.mainEndPoint), showSpinner);
  }
  ...
  public FindActivityReviewStatistics(activityId: any, showSpinner: boolean = false) {
    if (environment.IsDesignerVesion) {
      return of(TestDataService.ReviewStat).pipe(delay(500));
    }
  ...
  public AddReview(review: any, showSpinner: boolean = false) {
