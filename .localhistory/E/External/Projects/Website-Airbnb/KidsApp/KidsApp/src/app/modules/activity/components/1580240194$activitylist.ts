...
export class ActivityList extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public LoadActivities(pageIndex: number = 1) {
    ...
    this.IsLoading = true;
    this.activitySVC.FindAllActivities(this.FinalCriteria, this.Pager.PageIndex, this.Pager.PageSize, 'CreateDate DESC, Id DESC', this.ActivityInclude.join(','), false).subscribe((res: any) => {
  ...
  public OpenMapMark(item: any, context: any) {
    ...
    let box = new InfoBox({
      content:
        ...
        '<a href="https://www.google.com/maps/dir//' + item.Lat + ',' + item.Lng + '" value="Get directions" class="btn_infobox_get_directions" target="_blank">Directions</a>' +
