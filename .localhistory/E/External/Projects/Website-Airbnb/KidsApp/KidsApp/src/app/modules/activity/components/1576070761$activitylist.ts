  ...
  public ngOnInit() {
    ...
    this.route.queryParams.subscribe((res: any) => {
      ...
      if (res) {
        ...
        if (res.from && res.to) {
          var dateFom = new Date(res.from);
          dateFom.setDate(dateFom.getDate() + 1);

          var dateFomTo = new Date(res.to);
          dateFomTo.setDate(dateFomTo.getDate() + 1);


          this.Criteria.DateRangeTemp = [dateFom, dateFomTo]


          setTimeout(() => {
            //  $('#dpRange').val(res.from + ' - ' + res.to);
          }, 100)
        }
