...
export class SupplierInfo extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public OnDayClick(daysSelection: any) {
    ...
    let day = this.DaysSelections.filter((x: any) => x.Day == daysSelection.Day)[0]

    if (day.WorkingDay && day.WorkingDay.Id) {
      if (daysSelection.IsSelected) {
        ...
        daysSelection.WorkingDay = daysSelection.End = null;
      }
    }
    else if (day.WorkingDay) {
      if (daysSelection.IsSelected) {
        day.WorkingDay = { UserId: this.User.Id, State: AppEnums.BaseState.Added };
      }
      else {
        day.WorkingDay = null;
      }
  ...
  public ValidateWorkingDays() {
    ...
    this.DaysSelections.forEach((x: any) => {
      if (x.WorkingDay) {
        ...
        if (!this.User.WorkingDays.Start)
          x.StartError = true;
        if (!this.User.WorkingDays.End)
          x.EndError = true;

        if (!x.StartError && !x.EndError) {
          let StartDate = CoreHelper.TimeToDate(this.User.WorkingDays.Start);
          let EndDate = CoreHelper.TimeToDate(this.User.WorkingDays.Start);
