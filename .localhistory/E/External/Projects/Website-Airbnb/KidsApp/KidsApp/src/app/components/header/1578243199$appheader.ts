 ...
 ****************************/
  ...
  public OnUploadFile(event: any, type: Files) {

    //if (.AllowedDocumentExtension.toLowerCase().indexOf((file[0].name.split('.').pop()).toLowerCase()) == -1) {
    ...
    switch (type) {
      case Files.PerosnalPhoto:
        ...
        if (event.target.files.length == 0 || !event.target.files[0]) {
          ...
          if (photo) {
            photo.File = this.ConvertToBase64(event.target.files[0]);
          }
          else {
            this.User.UserDocument.push({
              File: this.ConvertToBase64(event.target.files[0]),
