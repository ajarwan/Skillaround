...
export class ActivityDetails extends BaseComponent implements OnInit, AfterViewInit {
  ...
  * ***************************************/
  ...
  public ReviewsPagger: Pager = new Pager(1, 6, 0, 0);
  ...
  public LoadActivityReviews() {
    ...
    this.activitySCV.FindAllReviews({ ActivityId: this.ActivityId }, this.ReviewsPagger.PageIndex, this.ReviewsPagger.PageSize
      , 'CreateDate DESC', this.ReviewsInclude.join(','), false).subscribe((res: any) => {
        this.Reviews = res.d;
        this.ReviewsPagger = res.pager;
