...
export class ActivityList extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public FindAllBordersMarks() {
    console.log(this.GmapVisible);

    let MapMarks = [];
    let MarksData = [{ Lat: '25.17284965631571', Lng: '55.23390376714974' }];
    ...
    MarksData.forEach((x: any) => {
      ...
      google.maps.event.addListener(marker, 'click', (() => {
        ...
        this.GmapVisible.setCenter(new google.maps.LatLng(x.Lat, x.Lng));
      }));
  ...
  public OpenMapMark(item: any) {


    let box = new InfoBox({
      content:
        '<div class="marker_info_2">' +
        '<img src="' + 'assets/images/thumb_map_hotel_6.jpg' + '" alt="Image"/>' +
        ...
        + '" type="hidden"><input type="hidden" name="daddr" value="' + item.Lat + ',' + item.Lng + '"><button type="submit" value="Get directions" class="btn_infobox_get_directions">Directions</button></form>' +
        '<a href="tel://' + '+3934245255' + '" class="btn_infobox_phone">' + '+3934245255' + '</a>' +
        '</div>' +
        '<a href="' + 'single_hotel.html' + '" class="btn_infobox">Details</a>' +
        '</div>',
    ...
    box.open(this.GmapVisible, this);
  }
