  ...
  *************************************/
  ...
  public DrawListStatistics(stat: any) {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      let labelTitle = chart.seriesContainer.createChild(am4core.Label);
      labelTitle.textAlign = "bottom";
      labelTitle.horizontalCenter = "bottom";
      labelTitle.verticalCenter = "bottom";
      labelTitle.adapter.add("text", (text, target) => {
        return `[font-size:18px]${this.resources.Total}[/]:\n[bold font-size:30px]` + chart.data[0].Total + "[/]";
      })
