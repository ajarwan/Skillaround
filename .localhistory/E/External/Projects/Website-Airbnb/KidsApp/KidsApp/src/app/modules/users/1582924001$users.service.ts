...
export class UsersService {
  ...
  public FindAllNotifications(criteria: any, pageIndex: number = -1, pageSize: number = -1, orderBy = null, include: string = null, showSpinner: boolean = true): Observable<Response> {
    ...
    return this.http.get(new UrlConfig(`${this.NOTIFICATIONS_BASE_URL}FindAll${params.getQuery()}`, this.mainEndPoint), false);
  }
  ...
  public OnConnectionApproval(userFriendId: number, status: AppEnums.AcceptStatus, showSpinner: boolean = false): Observable<Response> {
    var params = new SearchParams();
    params.set('userFriendId', userFriendId.toString())
    params.set('status', status.toString())
    return this.http.put(new UrlConfig(`${this.NOTIFICATIONS_BASE_URL}${params.getQuery()}`, this.mainEndPoint), showSpinner);
  }
